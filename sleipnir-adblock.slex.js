...
// @include         https://*
// @version         5.8.10
// @history         5.8.8 Hid ameblo's AmebaChoice widget, which the existing ameblo rules missed.
// @history         5.8.7 Hid ikarisokuhou's blog_ad slots and the Nicovideo Dictionary nicoad article-slide ads.
// @history         5.8.6 Hid nhentai's ts-im-ad-link slot, which the existing nhentai rules missed.
// @history         5.8.5 Added alfalfalfa.com's in-article ad slot and its livedoor blogroll ad to the per-site hide rules.
// @history         5.8.4 Fixed some matome sites (e.g. hero-news) going blank. A rule that hid any box containing an ad also hid the article when the ad sat inside the article box; that rule was removed. Ads are still hidden individually.
// @history         5.8.3 Fixed apexlegends-leaksnews / Obanew-style sites that could no longer scroll. Their interstitial ad locks the page while it shows, and blocking the ad left the lock in place. The lock is now released so the page scrolls again.
// @history         5.8.2 Reviewed and trimmed for fewer side effects. URL matching now uses the browser's URL parser, so ports, IPv6, and relative URLs are handled correctly; the window.open same-site check is stricter. Over-broad global hide rules (#ad/.ads/.sponsored/code-block) were dropped in favor of specific ones. Page-breaking hooks were narrowed or removed — JSON.parse editing is now YouTube-only, the back/forward (history/popstate) interception that broke SPA navigation is gone, the attachShadow override no longer forces shadow roots open, and consent/functional cookies are no longer deleted.
// @history         5.8.1 Added GameWith's PR / tie-up article tile to the per-site hide rules; it was missed when the other GameWith ad slots were covered in 5.7.1. Normal article tiles are left alone.
// @history         5.8.0 The EasyList fetch now also uses its network domain rules, not just the cosmetic hide selectors — so newly-listed ad hosts are blocked without waiting for a bundled-list update. Cached for the session like the cosmetic rules.
...
// @history:ja      5.8.10 プレミアムバンダイ(p-bandai.jp)で商品ページが表示されない不具合を修正。サイト側のボット判定が拡張機能の動作を誤検知して制限ページを出していたため、このサイトでは拡張機能を動かさないようにした。
// @history:ja      5.8.9 求人・EC・マンガ系サイトで使われていた広告/追跡ホスト(ma-tag 等)を追加。サイト本体の画像やCSSの配信元は誤って消さないよう除外した。
// @history:ja      5.8.8 アメブロの AmebaChoice ウィジェットを非表示に追加(既存の ameblo ルールで漏れていた分)。
// @history:ja      5.8.7 いかり速報の blog_ad 枠と、ニコニコ大百科の nicoad(記事スライド広告)を非表示に追加。
// @history:ja      5.8.6 nhentai の ts-im-ad-link 広告枠を非表示に追加(既存の nhentai ルールで漏れていた分)。
// @history:ja      5.8.5 アルファルファモザイクの記事内広告枠と livedoor blogroll 広告をサイト別非表示ルールに追加。
// @history:ja      5.8.4 一部のまとめ系サイト(hero-news 等)でページが真っ白になる不具合を修正。広告を含む箱を消すルールが、広告が本文の箱の中にある場合に本文ごと消していたため、そのルールを外した。広告は個別に非表示のまま。
// @history:ja      5.8.3 apexlegends-leaksnews / おばにゅー系サイトでスクロールできなくなる不具合を修正。これらのサイトは広告の表示中にページを固定するが、広告を消すと固定が残ったままだった。固定を解除してスクロールできるようにした。
// @history:ja      5.8.2 コードレビューを反映し、副作用を減らした。URL判定をブラウザのURLパーサに変更(ポート・IPv6・相対URLに正しく対応)、window.openの同一サイト判定を厳密化。広すぎたグローバル非表示ルール(#ad/.ads/.sponsored/code-block 等)を具体的なものに絞った。ページ機能を壊しうる介入を縮小・撤去 — JSON.parseの編集はYouTube限定に、SPAの戻る/進むを壊していた履歴介入を撤去、attachShadowの強制openを撤去、同意/機能cookieの削除をやめた。
// @history:ja      5.8.1 GameWith のPR(タイアップ広告)記事タイルをサイト別非表示ルールに追加。5.7.1 で他のGameWith広告枠を対応した際に漏れていた分。通常の記事タイルはそのまま。
// @history:ja      5.8.0 EasyList の取得で、要素非表示セレクタだけでなくネットワーク遮断ルールも使うようにした。新しく登録された広告ホストを同梱リストの更新を待たずに遮断できる。非表示ルールと同様にセッション中キャッシュする。
...
(function () {
  ...
  /* Bot-protection safelist: sites using Akamai Bot Manager (botmanager-
     browser-agent.js) flag native-function tampering (toString proxy /
     fetch-XHR proxy below) as a bot signal and serve a restriction page,
     making product pages unviewable. Skip the script entirely on these
     first-party shops (their own ad surface is negligible). */
  var BOT_PROTECTED_HOSTS = ['p-bandai.jp'];
  for (var _bp = 0; _bp < BOT_PROTECTED_HOSTS.length; _bp++) {
    var _bph = '.' + BOT_PROTECTED_HOSTS[_bp];
    if (hostname === BOT_PROTECTED_HOSTS[_bp] ||
        (hostname.length > _bph.length &&
         hostname.slice(hostname.length - _bph.length) === _bph)) {
      return;
    }
  }
  ...
  var AD_DOMAINS = [
    ...
    'doujinlist.com',
    'kpia.shop',
    /* 2026-04-30 audit: domains found in JP corpus HARs but missing here.
       clb.yahoo.co.jp / dsb.yahoo.co.jp intentionally excluded — Yahoo
       News uses them for lazy-load telemetry that gates article rendering. */
    'fundingchoicesmessages.google.com', 'adtrafficquality.google',
    'zucks.co.jp', 'zucks.net', 'k.zucks.net',
    ...
    'hera.d2c.ne.jp',
    'id5-sync.com', 'cdn.id5-sync.com',
    'browsiprod.com', 'cdn.browsiprod.com', 'events.browsiprod.com',
    'match.adsrvr.org',
    'tapad.com', 'pixel.tapad.com',
    'lijit.com', 'ap.lijit.com',
    's-onetag.com', 'signal-metrics-collector-beta.s-onetag.com',
    'ad-delivery.net',
    'analytics.twitter.com',
    /* 2026-05-01 audit: corpus replay survivors → block at network level. */
    'yads.c.yimg.jp',
    'casalemedia.com',
    'bidswitch.net',
    'flux-cdn.com',
    'rumcdn.geoedge.be', 'geoedge.be',
    /* path-based entries (require isAdUrl path-match fallback below) */
    't.co/i/adsct',
    'www.facebook.com/privacy_sandbox/pixel',
    'cdn.jsdelivr.net/gh/prebid',

    /* 2026-05-01 audit (round 2): retired-HAR + ad-heavy JP blog network. */
    'admatrix.jp', 'ad.poly.admatrix.jp',
    'polymorphicads.jp', 'img.polymorphicads.jp',
    'meas.ad.pr.ameba.jp',
    '4dex.io', 'c.4dex.io',
    'ups.analytics.yahoo.com',
    'semasio.net', 'uipglob.semasio.net', 'sg.semasio.net',
    'a-mo.net', 'sea1-sync.a-mo.net',
    'nexx360.io', 'fast.nexx360.io',
    /* path-based entries (round 2) */
    'stat.amebame.com/pub/ads',
    'sy.ameblo.jp/sync',
    'cdn.jsdelivr.net/npm/prebid',
    'www.temu.com/api/adx',

    /* 2026-05-01 audit (round 3): live scan of 6 ad-heavy JP sites
       (gamewith/game8/alfalfalfa/jin115/gendai.media/nikkansports). */
    /* DSP / SSP global (suffix) */
    'adsrvr.org',
    'dotomi.com', 'smartadserver.com', '360yield.com', 'adkernel.com',
    'contextweb.com', 'yieldmo.com', 'sharethrough.com', 'loopme.me',
    'moloco.com', 'blismedia.com', 'goldspotmedia.com', 'btloader.com',
    'dns-finder.com', 'fastclick.net', 'mfadsrvr.com', 'pmbmonetize.live',
    'rtbhouse.com', 'creativecdn.com', '2mdn.net', 'teads.tv', 'mathtag.com',
    'gumgum.com', 'amxrtb.com', 'amx1.net', 'simpli.fi', 'richaudience.com',
    'fwmrm.net', 'zemanta.com', 'outbrainimg.com', 'advolve.io', 'adtarget.biz',
    'durationmedia.net', 'deepintent.com', 'trustedstack.com', 'ymmobi.com',
    'sportradarserving.com', 'intentiq.com', 'smaato.net', 'unrulymedia.com',
    'playdigo.com', 'betweendigital.com', 'confiant-integrations.net',
    'privacymanager.io', 'appier.net', 'pippio.com', 'quantserve.com',
    'inmobi.com', 'liftdsp.com', 'adrecover.com', 'anymind360.com',
    'img-c.net', 'lmadps.jp', 'mediago.io',
    /* DMP / data / brand-safety */
    'rlcdn.com', 'crwdcntrl.net', 'doubleverify.com', 'media.net',
    'opecloud.com', 'ccgateway.net', 'travelaudience.com',
    /* JP-specific (specific subdomain to avoid blocking parent service) */
    'a.flux.jp',
    'rtbrain.app', 'adpushup.com',
    'speee-ad.jp', 'speee-ad.akamaized.net',
    'gsspcln.jp', 'genieesspv.jp',
    'b99.yahoo.co.jp',
    'cdp.livedoor.com', 'counter2.blog.livedoor.com', 't.blog.livedoor.jp',
    'ipcheck.blogsys.jp',
    'cadmus2.script.ac',
    'imp-bidapi.i-mobile.co.jp', 'spimgv1.i-mobile.co.jp',
    'spnativeapi-direct.i-mobile.co.jp', 'spnativeapi-tls.i-mobile.co.jp',
    'afl.rakuten.co.jp', 'xml.affiliate.rakuten.co.jp',
    /* path-based */
    'serv.ds.kakao.com/adx',
    'px.ads.linkedin.com',

    /* 2026-05-01 audit (round 4): live scan of 16 sites
       (newspapers + business mags + nlab + yaraon). */
    /* DSP / SSP / cookie-sync (suffix) */
    'syncingbridge.com', 'eyeota.net', 'ad-m.asia',
    'relaido.jp', 'piano.io', 'shinobi.jp', 'bance.jp',
    'liadm.com', 'caprofitx.com', 'connatix.com', 'nrich.ai',
    '4dex.tech', 'insiad.com', 'primis.tech', 'gsspat.jp',
    'indexww.com', 'hubvisor.io', 'agkn.com', 'w55c.net',
    'yellowblue.io', 'admanmedia.com', 'gmossp-sp.jp',
    'measureadv.com', 'sparteo.com', 'sitescout.com',
    'everesttech.net', 'colossusssp.com', 'openxcdn.net',
    'ipredictive.com', 'krushmedia.com', 'turn.com',
    'imrworldwide.com', 'smartadhi.com', 'springserve.com',
    'tremorhub.com', 'fireworktv.com', 'd2-apps.net',
    'monetixads.com', 'ocmthood.com', 'bricks-co.com',
    'mulan.cloud', 'sp-trk.com', 'pushmaster-cdn.xyz',
    'ust-ad.com', 'mediarithmics.com', 'omnitagjs.com',
    'chartbeat.com', 'chartbeat.net', 'exelator.com',
    'macromill.com', 'npttech.com', 'slim02.jp',
    'smartnews-ads.com', 'pa-cd.com',
    'fwpub1.com', 'fwpixel.com',
    /* Yahoo / Google / LINE / others (host-precise) */
    'apm.yahoo.co.jp', 'cksync.yahoo.co.jp', 'im.c.yimg.jp',
    'imasdk.googleapis.com',
    'fluctssp.smt.docomo.ne.jp',
    'tr.line.me',
    ...
    /* 2026-05-01 audit (round 5): live scan extended to 30 sites
       (added IT/gaming/lifestyle/job/matome). */
    /* DSP / SSP / cookie-sync / DMP (suffix) */
    '3lift.com', 'presage.io', '1rx.io', 'demdex.net', 'bidr.io',
    'stackadapt.com', 'uncn.jp', 'admatic.de', 'seedtag.com', 'clvrads.com',
    'nextmillmedia.com', 'onetag-sys.com', 'sp-gn.com', 'servenobid.com',
    'cootlogix.com', 'bfmio.com', 'ingage.tech', 'nr-data.net',
    'aiasahi.jp', 'adobedtm.com', 'omtrdc.net', 'clmbtech.com',
    'aralego.com', 'presco.asia', 'company-target.com', 'tiktokw.us',
    'brand-display.com', 'adotmob.com', 'cnobi.jp', 'seenthis.se',
    'onesignal.com', 'nowads.com', 'adsboosters.xyz', 'zimg.jp',
    'cloudflareinsights.com', 'rfihub.com',
    /* host-precise */
    ...
    /* round 6 re-scan 2026-06-20 (job/EC): content CDN excluded */
    'ma-tag.com', 'pm-serv.co', 'a-mpd.com', 'tk.csolution.jp',
    'prismpartner.smt.docomo.ne.jp',
    /* path-based (round 6) */
    'www.google.com/rmkt/collect',
    'www.google.com/pagead', 'www.google.co.jp/pagead',
    ...
    'stickyadstv.com', 'htlbid.com', 'kueezrtb.com',
    'metrica-yandex.com', 'cdn-adtech.com', 'tradeadexchange.com',
    'uidsync.net', 'adblade.com', 'genieedmp.com', 'genieessp.com',
    'adscale.de', 'adnxs.net', 'adnxs1.com', 'adnxs-simple.com',
    'googleadservices-cn.com', 'propellerads.tech',
    'safesync.com', 'm-rtb.com',
    /* Adult ad networks */
    'trafficjunky.net', 'ero-advertising.com', 'adultadvertising.net',
    /* Mobile ad SDKs (in-app WebView coverage) */
    'applovin.com', 'applvn.com', 'unityads.unity3d.com', 'unityads.com',
    'ironsrc.com', 'ironsrc.mobi', 'vungle.com', 'chartboost.com',
    'tapjoy.com', 'tapjoyads.com', 'mintegral.com', 'liftoff.io',
    'pangle.io', 'adgatemedia.com', 'fyber.com', 'inner-active.mobi',
    'flurry.com', 'startapp.com',
    /* round 8 — live scan of 66 sites (user-directed + manga + overseas gaming + EC + SNS) */
    /* Overseas gaming SSP/DSP cluster */
    't.adx.opera.com', 't.oa.opera.com',
    '33across.com',
    'smilewanted.com',
    'rbstsystems.live', 'iqzonertb.live',
    'pbs.optidigital.com', 'opti-digital.com',
    'eskimi.com',
    'sundaysky.com',
    ...
    'infolinks.com',
    'sentinelpro.com',
    'cm-exchange.toast.com', 'cm.nhnace.com',
    'rqtrk.eu',
    'pgammedia.com',
    'aditude.io', 'aditude.cloud',
    'vidazoo.com',
    'clickagy.com',
    'sonobi.com',
    'pubeasy.io',
    ...
    'cookielaw.org',
    'cookiebot.com',
    /* path-based — keep host alive for non-ad fetches */
    'wikia-services.com/__track',
    'i.viafoura.co/v3',
    'pinimg.com/ct',
    'ct.pinterest.com/v3',
    'pbs.yahoo.com/setuid',
    'disqus.com/redirectuser',
    '.run.app/events?cee=no',
    /* round 9 — live scan of 15 webtoon/manga + overseas image-board/matome */
    'onetrust.com',
    'intergient.com', 'intergi.com',
    ...
    'marriedmailbox.com',
    'webbyword.net',
    'appsflyersdk.com', 'onelink.me',
    'tivan.naver.com', 'veta.naver.com',
    'ntv.io',
    'capig.stape.jp',
    'sync.adtech.ink',
    'sync.adtelligent.com',
    /* path-based — host has non-ad routes */
  ...
  var SITE_RULES = {
    ...
    'apexlegends-leaksnews.com': [
      ...
      'div[id^="smarttag_"]',
      'div[id^="eot-"]'
      /* .ai-viewport-1/2/3 was intentionally removed in 5.5.5
         Advanced Ads uses these as full-width scroll/visibility listeners.
         Hiding them with display:none kept the listeners alive but on a
         re-generated host element, swallowing taps on the whole page. */
    ],
    'ddd-smart.net': [
      'iframe.ddddnp', 'iframe.dddnp',
      'iframe[src*="mavrtracktor.com"]',
    ...
    'nhentai.net': [
      '.advertisement', '.advt', '.ts-im-ad-link',
    ...
    'dic.nicovideo.jp': ['.ad-bannar-maincolumn-top', 'div[id^="crt-"]', '[class*="nicoad" i]', 'iframe[src*="ads.nicovideo"]'],
    ...
    'ameblo.jp': ['.subAdBannerHeader', '.subAdBannerArea', 'div[data-slot="injected"]', 'div[amb-component="entryAd"]', '.bfl-snews__outer', '.skin-entryAd', '[class*="AmebaChoice" i]'],
    ...
    'gamewith.jp': ['#ad-jack', '.gwt-ad-jack', '.home-header-ad-notice-wrapper',
      ...
      '.gamedb-steam-ad-compact', '.gdb-feature_tile-item.is-pr'],
    /* syosetu.com (小説家になろう, ncode.* もサブドメイン suffix で適用):
       BEM の c-ad 系。広告専用命名で誤爆リスク低。実DOMで存在確認 (2026-05-24)。 */
    'syosetu.com': ['.c-ad', '[class^="c-ad--"]'],
    /* alfalfalfa (アルファルファモザイク): GAM 記事内枠と livedoor blogroll 広告。 */
    'alfalfalfa.com': ['.gam-ad-inline', '[class*="blogroll_ad"]'],
    'ikarisokuhou.blog.jp': ['[class^="blog_ad"]']
  ...
  /* apexlegends-leaksnews / おばにゅー系: interstitial 広告の表示中だけ html へ
     gn_inst_scroll_cancel (position:fixed) を付けて背景スクロールを止める。
     広告本体は遮断するが、ユーザーが押すはずの閉じる(×)が無くなるため class が
     残り続け、ページが永久にスクロール不能になる。固定を打ち消して復活させる。 */
  SLEX_addStyle('html.gn_inst_scroll_cancel { position: static !important; top: auto !important; left: auto !important; height: auto !important; min-height: 0 !important; overflow: visible !important; }');
  ...
  /* 「広告を直下に持つ親 div」を一括で消す div:has(> 広告) 処理は撤去。
     広告が本文コンテナ直下にあるサイト(例 hero-news の .content は直下に
     ins.adsbygoogle を持つ)で本文ごと消して白画面化するため。広告本体は
     個別セレクタで非表示済み、空枠の CLS は killAdSizedEmpties でカバーする。 */
  ...
  var CACHE_NET_KEY = '_sab_fn';
  var CACHE_TTL = 6 * 60 * 60 * 1000;

  function applyNetCache(str) {
    if (!str) return;
    var arr = str.split(',');
    for (var k = 0; k < arr.length; k++) { if (arr[k]) AD_HOST_SET[arr[k]] = true; }
  }
  ...
  function loadExternalFilters() {
    ...
    try {
      ...
      if (cached && ts && (now - ts) < CACHE_TTL) {
        ...
        applyNetCache(sessionStorage.getItem(CACHE_NET_KEY));
        return;
      }
    } catch (e) {}
    ...
    try {
      ...
      if (resp) {
        ...
        for (var i = 0; i < lines.length; i++) {
          var l = lines[i];
          /* network rule ||host^ (no options/exception/wildcard): feed the
             same AD_HOST_SET index isAdUrl uses, alongside bundled hosts and
             the filters.json delta. Caret must be the last char so options
             like $third-party are skipped (those are conditional). */
          if (l.charCodeAt(0) === 124 && l.charCodeAt(1) === 124) {
            if (netCount < 60000) {
              var caret = l.indexOf('^');
              if (caret > 2 && caret === l.length - 1) {
                var host = l.substring(2, caret);
                if (host && !/[*\/]/.test(host) && /^[a-z0-9][a-z0-9.\-_]*$/.test(host)) {
                  AD_HOST_SET[host] = true;
                  netHosts.push(host);
                  netCount++;
                }
              }
