...
// @include         https://*
// @version         5.8.4
// @history         5.8.4 Fixed pages going completely blank on hero-news and similar livedoor-matome sites. The div:has(> ad-child) rules — which hid any div that directly contained an ad — also hid the article body whenever the ad sits inside the content container (hero-news's .content holds an ins.adsbygoogle, so the entire article became display:none and the page was blank). Removed the div:has(> ad) parent-removal block entirely; the ads themselves are still hidden by their own selectors and empty shells are handled by killAdSizedEmpties. Verified on a real Sleipnir build: hero-news body restored (0 -> 9614 chars, still 0 visible ads), robosoku fine. Corpus 10/10 PASS. This was the parent-walk content-swallow class that earlier reviews had warned about.
// @history         5.8.3 Fixed apexlegends-leaksnews / Obanew-style sites being permanently unscrollable. Their interstitial ad locks the page by adding gn_inst_scroll_cancel (position:fixed) to <html>; the blocker removes the ad but the user-facing close button goes with it, so the class is never cleared and the page can never scroll again (verified on-device: this is the real cause of the "doesn't work on OW" report, present even before this fix). The override now neutralizes html.gn_inst_scroll_cancel (position:static / height:auto / overflow:visible) to restore scrolling. Verified on a real Sleipnir build: scrolling returns all the way to the page footer. Scoped to html.gn_inst_scroll_cancel, no effect on other sites; corpus unaffected (that class is absent from every corpus page).
...
// @history:ja      5.8.4 hero-news や livedoor まとめ系サイトでページが真っ白になる不具合を修正。広告を直下に持つ div を消す div:has(> 広告) ルールが、広告が本文コンテナの直下にある場合に本文ごと消していた(hero-news の .content は直下に ins.adsbygoogle を持つため記事全体が display:none になり白画面化)。div:has(> 広告) による親要素の一括削除を撤去。広告本体は個別セレクタで非表示のままで、空枠は killAdSizedEmpties が処理する。実機 Sleipnir で確認: hero-news の本文が復活(0 → 9614文字、表示広告は0のまま)、robosoku も正常。corpus 10/10 PASS。これは過去のレビューで警告されていた「親遡及による本文巻き込み」だった。
// @history:ja      5.8.3 apexlegends-leaksnews / おばにゅー系サイトが永久にスクロール不能になる不具合を修正。これらのサイトは interstitial 広告の表示中に <html> へ gn_inst_scroll_cancel(position:fixed)を付けて背景を固定するが、広告本体を遮断すると閉じる(×)ボタンも消えるため class が解除されず残り続け、ページが二度とスクロールできなくなっていた(実機で確認、これが「OWで動かない」報告の真因。本修正前から存在)。html.gn_inst_scroll_cancel の固定を打ち消す(position:static / height:auto / overflow:visible)ことでスクロールを復活。実機 Sleipnir でページ最下部までスクロール復活を確認。html.gn_inst_scroll_cancel 限定で他サイトに無影響(corpus 全ページに該当クラスは存在しないため影響なし)。
...
(function () {
  ...
  /* div:has(> 広告) で「広告を直下に持つ親 div」を一括で消す処理は撤去
     (v5.8.4)。記事本文コンテナ(例 hero-news の .content は直下に
     ins.adsbygoogle を持つ)を本文ごと消して白画面化する親遡及の巻き込みが
     実機で確認された。広告本体は個別セレクタ(ins.adsbygoogle / [id^=yads_]
     / iframe[src*=doubleclick] 等)で非表示済み、空枠の CLS は
     killAdSizedEmpties でカバーする。 */
